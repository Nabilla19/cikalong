...
export default async function ArsipDokumenPage() {
  ...
  const images = arsip.filter((item: any) => item.file_url?.toLowerCase().match(/\.(jpeg|jpg|gif|png|webp|svg)(\?.*)?$/) !== null);
  const documents = arsip.filter((item: any) => item.file_url?.toLowerCase().match(/\.(jpeg|jpg|gif|png|webp|svg)(\?.*)?$/) === null);
